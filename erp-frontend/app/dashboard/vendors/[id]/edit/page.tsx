...
export default function EditVendorPage() {
  ...
  useEffect(() => {
    const loadVendor = async () => {
      try {
        const data = await vendorsApi.getById(parseInt(id));
        setFormData(data);
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await vendorsApi.update(parseInt(id), formData);
        ...
        <div className="space-y-6">
          <Card className="rounded-2xl border-border/40 shadow-none bg-primary/5">
            ...
            <CardContent className="p-8 pt-4 space-y-6">
              ...
              <div className="space-y-2">
                <Label htmlFor="status" className="text-[10px] uppercase font-bold tracking-widest text-muted-foreground ml-1">Status</Label>
