        ...
        <div className="glass rounded-[3rem] p-8 md:p-12 shadow-[0_32px_64px_-12px_rgba(0,0,0,0.1)] border-white/20">
          <div className="flex flex-col items-center text-center">
            ...
            <p className="mt-3 text-sm font-medium text-muted-foreground">
              {step === 1 ? "Join businesses using SmartFlow ERP." : "Create a strong password for your workspace."}
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="mt-10 space-y-6">
              ...
              <div className="space-y-6 animate-in slide-in-from-right-4 duration-500">
                  ...
                  {formData.password && (
                    <div className="grid grid-cols-2 gap-3 p-4 bg-muted/20 rounded-2xl border border-border/50 mt-4">
                      {passwordRequirements.map((req, i) => (
                        <div key={i} className={cn("flex items-center gap-2 text-[10px] font-black uppercase tracking-wider transition-colors",
                          req.test(formData.password) ? "text-primary" : "text-muted-foreground/50")}>
                ...
                <div className="flex items-start gap-3 ml-1">
                  <input type="checkbox" id="terms" required
                    className="mt-1 h-5 w-5 rounded-lg border-border/50 bg-background/50 text-primary focus:ring-primary/20" />
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-4">
              ...
              <Button type="submit" className="flex-1 h-16 rounded-2xl text-lg font-black shadow-2xl shadow-primary/20 order-1 sm:order-2 active:scale-[0.98] transition-all"
                disabled={isLoading || (step === 2 && !allPasswordRequirementsMet)}>
                {isLoading ? (
                  <><Loader2 className="h-5 w-5 animate-spin mr-2" />Processing...</>
                ) : (
                  <>{step === 1 ? "Continue" : "Create Account"}<ArrowRight className="ml-2 h-5 w-5" /></>
                )}
              </Button>
            </div>
          </form>
          ...
          <div className="mt-10 text-center">
            <p className="text-sm font-medium text-muted-foreground">
              Already a member?{" "}
              <Link href="/login" className="font-black text-primary hover:underline underline-offset-4 tracking-tight">Sign In</Link>
            </p>
          </div>
        </div>
      </div>
